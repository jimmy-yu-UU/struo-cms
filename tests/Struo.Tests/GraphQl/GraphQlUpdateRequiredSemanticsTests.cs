...
/// <summary>
/// GraphQL parity for the update-path Required semantics: <c>updateRole</c> goes through the
/// same <see cref="Struo.Application.Query.ItemService.UpdateAsync"/> as REST, so omitting
/// <c>name</c> (Required=true) from an <c>updateRole</c> input succeeds and keeps the role's stored
/// name, exactly like the REST partial-update rule in <c>docs/guide/en/12-rest-conventions.md</c>'s
/// "What a `POST`/`PUT` body can set" section.
