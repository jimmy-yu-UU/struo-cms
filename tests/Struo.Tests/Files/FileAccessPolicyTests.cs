...
// the permission-related dependencies instead of FilesController holding them directly.
// CanReadUnpublished is a plain synchronous predicate over the already-resolved per-request
// identity/permission snapshot — the AuthSchemes.Adaptive policy scheme (see
// BearerReadAuthenticationTests) is what guarantees a bearer caller's identity and grants are
// resolved before this runs, so no scheme probing happens here.
