...
public class AuthLoginRateLimitTests(ApiFactory factory)
{
    ...
    public async Task Login_beyond_permit_limit_returns_429_with_error_envelope()
    {
        ...
        // OnRejected dispatches its message by which policy rejected the request (added
        // alongside the "password" policy); pin the exact literal here so a silent fall-through
        // to the generic "_" branch (e.g. if the PolicyName metadata lookup ever came back null
        // for this policy) shows up as a failure here instead of only in a documentation
        // transcript.
