...
const i18n = createI18n({
  ...
  fallbackLocale: 'en',
  messages: { en: { media: {
    folderName: 'Folder name', folderNameDescription: 'Enter a name for this folder.', folderConfirm: 'OK',
  } } },
})
...
describe('MediaFolderNameDialog', () => {
  ...
  it('renders a description, so reka does not warn about a dangling aria-describedby', async () => {
    ...
    const w = mount(MediaFolderNameDialog, {
      props: { visible: true, header: 'New folder' },
      global: { plugins: [i18n], stubs: { teleport: true }, renderStubDefaultSlot: true },
      attachTo: container,
    })
    await flushPromises()
