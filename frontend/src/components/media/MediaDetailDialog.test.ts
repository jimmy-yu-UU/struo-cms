...
describe('MediaDetailDialog', () => {
  ...
  it('loads mediafolder rows and shows a Folder TreeSelect', async () => {
    ...
    const w = mountDialog()
    await flushPromises()
    expect(itemsApi.list).toHaveBeenCalledWith('mediafolder', expect.objectContaining({ page: 0, rows: 500, sort: 'name' }))
    expect(w.findComponent({ name: 'TreeSelect' }).exists()).toBe(true)
  })

  it('selects the current folder as the initial TreeSelect value when item.folderId is set', async () => {
    vi.spyOn(itemsApi, 'get').mockResolvedValue({ ...item, folderId: 'b' } as never)
    const w = mountDialog()
    await flushPromises()
  ...
  it('selects the Uncategorized node as the initial TreeSelect value when item.folderId is null', async () => {
    vi.spyOn(itemsApi, 'get').mockResolvedValue({ ...item, folderId: null } as never)
    const w = mountDialog()
    await flushPromises()
    const vm = w.vm as unknown as { folderId: string | null }
