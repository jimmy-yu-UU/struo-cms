...
<template>
  <section class="effective-permissions">
    ...
    <p v-else-if="eff && rows.length === 0" class="notice text-muted-foreground">{{ t('rbac.effectiveEmpty') }}</p>
      ...
      <table class="panel-table">
        ...
        <tbody>
          <tr v-for="r in rows" :key="r.name">
            ...
            <td><Check v-if="r.read" class="mx-auto size-4" aria-hidden="true" /><span class="sr-only">{{ r.read ? t('rbac.colRead') : '' }}</span></td>
            <td><Check v-if="r.write" class="mx-auto size-4" aria-hidden="true" /><span class="sr-only">{{ r.write ? t('rbac.colWrite') : '' }}</span></td>
            <td><Check v-if="r.delete" class="mx-auto size-4" aria-hidden="true" /><span class="sr-only">{{ r.delete ? t('rbac.colDelete') : '' }}</span></td>
          </tr>
        </tbody>
      </table>
    </div>
