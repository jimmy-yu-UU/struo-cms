...
清單端點的查詢字串怎麼寫、篩選與排序怎麼組合、分頁怎麼算，REST 與 GraphQL 共用同一套語意，是這一章
的主題。

## 查詢怎麼寫

清單端點有兩種等價的請求形狀，背後是同一個 `QueryModel`：都由 `QueryParser` 解析，都在 SQL 組出來
之前先過 `QueryValidator` 的白名單。

**查詢字串。** `GET /api/items/{collection}` 讀 bracket 語法的重複鍵：
`filter[<欄位或關聯路徑>][<運算子>]=<值>`，加上 `sort=`、`limit=`、`offset=`、`search=`、`fields=`、
`deep=`、`facets=`、`aggregate[<op>]=`，以及控制器層級另外讀的 `locale=` 與 `deleted=`。一次過濾、
一個遞減排序、外加分頁：
...
只有一篇已發佈的文章排在前面被回傳；`meta.total` 是 2，代表符合篩選的總列數，`limit`／`offset` 則是
夾完之後真正生效的分頁值。

**JSON 封裝。** `POST /api/items/{collection}/query` 收下同一個模型當 JSON body：`filter`、`sort`、
`limit`、`offset`、`fields`、`deep`、`search`、`facets`、`aggregate`。這是唯一能寫 `_and`／`_or`、
也是唯一能替一個 `deep` 關聯自己的 `fields`／`filter`／`sort`／`limit`／`offset`，或再巢狀一層
`deep` 的方式。上面那個查詢寫成 JSON body 是同一件事：
...
`_eq` 換成 `eq`，量詞從 `_some` 換成 `some`——拼法留給 GraphQL 一章。

**兩者都是讀。** 兩個動作都沒有 `[Authorize]`，`ItemService.QueryAsync` 只檢查 `CanRead`；
`POST .../query` 雖然用 POST，也不需要寫入授權。但只要呼叫端是靠 session cookie，`POST .../query`
一樣要帶 `X-Struo-CSRF` 標頭——CSRF 規則只看 HTTP 方法，不看是不是讀取，一個純讀的 POST 跟一般寫入
被擋的方式相同。

`locale=` 與 `deleted=` 一律從網址讀，即使在 `POST .../query` 上也是，所以 JSON 封裝形式要寫成
`POST /api/items/article/query?locale=zh-TW&deleted=only`。
...
- **`sort`**——依一或多個欄位排序，見下方「排序」。
- **`limit`／`offset`**——分頁，見下方「分頁與 `meta`」。
- **`fields`**——只留下集合自己的欄位，留給進階查詢的一章。
- **`deep`**——展開關聯，同樣留給進階查詢的一章；第 8 章已經先講過批次展開的機制，見
  [第 8 章：關聯](08-relations.md)。
- **`facets`／`aggregate`**——在同一批過濾結果上算摘要統計，留給進階查詢的一章。

## 運算子

`QueryOperator` 定義的每一種運算子，各自對應查詢字串／JSON 的一個 token，也對應一個 GraphQL 拼法：
...
| `_gt` | `gt` | 大於 |
| `_gte` | `gte` | 大於等於 |
| `_null` | `isNull: true` | 為 null |
| `_nnull` | `isNull: false` | 不為 null |
| `_contains` | `contains` | 包含子字串 |
| `_starts_with` | `startsWith` | 以子字串開頭 |
| `_ends_with` | `endsWith` | 以子字串結尾 |

**比對區分大小寫。** 運算子 token 是大小寫敏感的比對，`_EQ` 或 `_startsWith` 會被拒絕，回
`Unknown operator '<token>'.`；欄位名稱則相反，到處都不分大小寫。

**接受值的形狀。** `_in`／`_nin` 在查詢字串上是逗號清單，在 JSON 裡是陣列，兩種形狀最後都會併成同一
份 SQL `IN` 清單。`_contains`／`_starts_with`／`_ends_with` 分別轉成 `LIKE '%v%'`、`LIKE 'v%'`、
`LIKE '%v'`。
...
**型別轉換。** 除了 `_null`／`_nnull` 以外，每個運算子的值都會轉成該欄位真正的 CLR 型別再比對——
`Guid` 外鍵用 `uuid = uuid` 比，不是文字比對；轉換表涵蓋 `Guid`、`int`、`long`、`short`、`bool`、
`DateTime`、`DateTimeOffset`、`decimal`、`double`、`float`，`string` 欄位與任何沒被涵蓋的型別維持
不轉型的文字比較。值一律用不隨地區變動的格式送出，主機自己的預設文化特性也在啟動時固定成
invariant，兩邊才不會對同一個小數或日期讀出不同結果。

**沒有介面限制。** 查詢 DSL 不會拿運算子配欄位介面：對一個 `Text` 欄位下 `_gt` 一樣會被接受，只是
當成文字比較，不是錯誤。
...
{"filter":{"_or":[{"status":{"_eq":"draft"}},{"categoryId":{"_eq":"<Guides>"}}]}}
```

三篇文章都符合：草稿那一篇滿足第一個分支，三篇的分類都是 `Guides`，滿足第二個分支——`meta.total`
是 3。
...
**其他形狀錯誤。** 一個空的 `filter` 物件是錯誤（`Empty filter object.`），完全不帶 `filter` 鍵則是
合法的無操作；一個欄位的值必須是運算子物件，`{"status":"draft"}` 會被拒絕成
`Filter for field 'status' must be an object of operators.`，`{"status":{}}` 則是
`Filter for field 'status' has no operator.`；`_and`／`_or` 的值必須是陣列：
`'_or' must be an array.`；查詢字串上一個沒有分成欄位與運算子兩段的鍵是
`Malformed filter key 'filter[status]'.`
...
**逐段成立。** 一條帶點號的路徑，兩個條件可以各自被不同的關聯列滿足：

```text
$ GET /api/items/article?filter[tags.name][_eq]=howto&filter[tags._junction.note][_eq]=hero
```

回傳 A 與 B 兩篇——只要文章有任何一個標籤叫 `howto`、以及任何一個標籤的連結備註是 `hero` 就算數，
不要求同一個標籤兩個條件都成立。
...
只剩 A：A 身上「名字是 `howto`」跟「連結備註是 `hero`」落在同一個標籤連結上；B 雖然也有 `howto`
標籤，但備註是 `hero` 的是另一個標籤。`_some`／`_none` 對多對一關聯一樣合法：`_some` 等於同一個內
層篩選的逐點路徑，`_none` 則是「沒有符合的目標，或外鍵本身是 null」兩種情況都算。
...
一層 `_and`／`_or`。查詢字串上量詞是路徑裡的一個片段——`filter[tags._some.name][_eq]=howto`——同一
個（前綴、量詞）底下的條件會先被分組再折成一個量詞，前綴比對不分大小寫；同一個請求裡每個（前綴、量
詞）只能有一組，兩個各自獨立的同列條件要用 JSON 封裝表達，例如
...
- 兩個量詞不能相鄰：`'tags._some._none.name': two quantifiers in a row are not allowed.`
- 量詞也不能出現在一般的欄位路徑裡：
  `'tags._some.name': relation quantifiers are not valid inside a field path here.`

**`_junction`。** `_junction.<欄位>` 篩的是連結本身的 payload，不是任一端的資料，可以逐點使用，也可
以放進 `_some`／`_none` 裡。它必須緊跟在一個帶 junction collection 的多對多關聯之後，後面只能接一
個非隱藏的 payload 欄位、不能再接下一段；用在多對一上會被拒絕：
`'category._junction.note': '_junction' is only valid after a many-to-many relation with a
junction collection.` `_junction` 需要呼叫端對 junction collection 本身的讀取授權，這個檢查在欄位
名稱被解析之前就先做，讀不到的 junction 不會被拿來反推有哪些欄位。它不算進關聯深度上限，因為只是
一個附加片段；`_junction` 條件也不能跟目標集合的條件在同一個量詞裡用 OR 接起來。

**軟刪除的界線。** 關聯子查詢永遠套用目標集合自己的軟刪除過濾，不管外層請求的 `deleted=` 是什麼；
即使正在看父層的垃圾桶，一筆已丟進垃圾桶的關聯資料也不會滿足帶點號路徑或量詞條件。

## 可翻譯欄位怎麼被篩選

十三個運算子在可翻譯欄位上全部可用：篩選會被改寫成一條對翻譯 sidecar 的子查詢，在有效的查詢
locale 下比對。不用特別帶 `locale=`——沒給的話用站台預設值：

```text
$ GET /api/items/article?filter[title][_contains]=start
```

不帶 `locale=` 時用站台預設（`en`）比對，回傳標題含 `start` 的那一篇，`meta.total` 為 1。換成中文
標題、明白指定 locale：

```text
$ GET /api/items/article?filter[title][_contains]=%E9%96%8B%E5%A7%8B&locale=zh-TW
```

回傳的是同一篇文章，`translations` 只剩 `zh-TW` 一筆，`meta.total` 一樣是 1。
...
有效查詢 locale 一律算成站台預設值，即使根集合自己沒有可翻譯欄位，也會為了關聯路徑上可能碰到的可
翻譯集合而算出這個值。
...
**沒有 `sort=` 時。** 預設順序是 `createdAt DESC, id ASC`，沒有 `CreatedAt` 的集合退化成單獨的
`id ASC`；這是刻意的——PostgreSQL 的堆積順序在更新後不穩定，不這樣做的話一筆被編輯過的列會憑空跳
位置。主鍵一律被加在每個客戶端排序的最後一段當遞增的平手判定，除非呼叫端已經自己排 `id`，這樣一來
就算排序欄不是唯一值，翻頁也不會重複或漏掉列。

排序一個可翻譯欄位會對翻譯 sidecar 開一條 locale-scoped 的關聯子查詢；某個 locale 沒有翻譯列的那一
列排序值當成 `NULL`。

排序鍵套的白名單跟篩選欄位是同一份，細節留到本章最後一節。

## 分頁與 `meta`

分頁只有 `limit`／`offset` 兩個參數，沒有 `page`。`limit` 沒給或不是正數會被夾到
`Query:DefaultLimit`（預設 25），超過上限的會夾到 `Query:MaxLimit`（預設 100）；`offset` 最低是 0。
`limit=abc` 這種非數字值不會報錯，直接當成沒給處理，一樣被夾到預設值：

```text
$ GET /api/items/article?limit=abc
```

回應的 `meta` 是 `{"total":3,"limit":25,"offset":0}`——`limit` 落回預設的 25，不是拒絕請求。
...
`deleted=` 是同一個查詢介面的一部分，在清單、`POST .../query` 與單筆讀取上都一樣，一律從網址讀，含
`POST .../query`。三個可接受值、預設值與權限規則見
[第 9 章：版本紀錄與軟刪除](09-revisions-and-trash.md)「`deleted=` 過濾器與權限」一節。

這道權限檢查寫在控制器上，不在服務層：直接呼叫 `ItemService.QueryAsync`／`GetAsync` 的呼叫端只會得
到 `CanRead` 檢查，不會自動套用 `deleted=` 的授權門檻。
...
隱藏欄位刻意被排除在白名單之外：一個看起來像密碼的隱藏欄位如果還能拿來篩，`meta.total` 就會變成一
個逐字元試出內容的側錄工具。一個不存在的自己欄位不管像不像打錯字都用同一種方式拒絕：
...
絕——`FORBIDDEN`，匿名呼叫端則是 `UNAUTHORIZED`——不是悄悄把那個條件拿掉。拿掉條件而不是拒絕請求，
會讓呼叫端拿到不符合它送出的篩選的資料，而且就算真的回傳零筆，`meta.total` 本身在一個讀不到的集合
上也是一種側錄工具。授權檢查在路徑的葉欄位被解析之前就先做，所以一個讀不到的集合上真正存在的欄
位，跟編造出來的欄位，在這裡拿到的是同一種拒絕。

多對一外鍵欄位本身維持可篩：它是呼叫端本來就能讀到的那一列上的一欄，而且只暴露一個不透明的 id，不
會洩漏目標集合的任何資料。

一個集合被列進 `Rbac:PublicReadCollections`，如果它的公開篩選條件或 `deep=` 會碰到第二個集合，那
第二個集合也要一併列進去，否則就會撞上上面的授權拒絕。

錯誤信封的完整形狀留給 REST API 慣例那一章。

## 接下來

篩選、排序、分頁都清楚之後，下一步是怎麼投影欄位、展開關聯、算 facet 與彙總，這是進階查詢一章的主
題。
