...
密碼雜湊只有一種實作：Argon2id，參數寫死在程式裡，時間成本 3、記憶體成本 65536（64 MiB）、平行
度 1、混合定址、輸出 32 位元組，產生一個自我包含的 PHC 編碼字串；鹽值與參數都內嵌在雜湊本身，
`user` 這個集合上並沒有另外一欄鹽值。
...
原則回 `400`，附上欄位訊息。一個只透過 OIDC 登入、從沒設過本機密碼的帳號，本人呼叫時會在驗證
目前密碼之前被擋下，回 `400` `NO_LOCAL_PASSWORD`；超級管理員仍然可以替這樣的帳號另外設一個
本機密碼。整個身分相關的介面裡，只有這個端點容許非超級管理員寫入。
...
`/graphql`、檔案讀取這些沒有標明任何 scheme 的端點，也認得出一個 bearer 呼叫端。在這些沒有標明
scheme 的端點上，壞掉或已撤銷的 token 一律降級成匿名，不會退回去試 cookie，即使這次請求同時帶
著兩者；要求登入的寫入端點則兩種 scheme 都會被驗證再合併，見
...
節會說明，使用者層級的撤銷（改密碼、刪除帳號）靠這個索引，所以原地續期出來的工作階段，只會被前
一個使用者的改密碼或刪除觸發撤銷，不會被新使用者的觸發撤銷。換帳號之前先登出，別讓同一個 cookie
jar 同時對應兩個帳號。
