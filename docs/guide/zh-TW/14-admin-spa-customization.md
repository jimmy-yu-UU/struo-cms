...
標記、對齊方式、區塊轉換、連結指令、水平線、圖片,以及復原/重做——再加上夾在登記表兩個區段之間、
卻從來沒有加入登記表本身的標題層級下拉選單與插入表格的彈出視窗,因為下拉選單與帶尺寸的網格彈出視
窗需要的介面,不是登記表那個單一的 `run(editor, ctx)` 函式能表達的。它一直顯示在畫面上,不需要有
選取範圍才存在。
...
`/` 才會打開選單(`allowedPrefixes` 維持上游預設值 `[' ']`,沒有動它)。這正是為什麼 "and/or" 或一段
URL 路徑不會打開選單:兩種情況下,`/` 都夾在同一個 text node 裡的兩個普通字元中間。

"什麼都沒有"這件事,比聽起來還要窄,而且值得明講成一個已知限制,而不是刻意設計:因為上游量測的邊界
是 text node 而不是區塊,一個剛好打在某個 mark(例如粗體或連結)結束、普通文字接續起來那個位置的
`/`,一樣會打開選單——那個位置正是一個全新 text node 的 offset 0,前面沒有任何字元可以讓前綴檢查
拒絕它。使用者如果緊接在一段粗體文字之後、中間沒有空格地輸入 `/`,就會看到選單打開,這可能會讓人
意外。這是 `@tiptap/suggestion` 比對前綴的方式帶來的結果,不是 StruoCMS 這個 extension 加上去的
規則,也沒有辦法在不同時破壞「行首」那個同一條規則要保留的情況下,把它擋掉。
...
`group: 'block'` 的項目(項目符號清單、編號清單、引言區塊、程式碼區塊);以及一個手寫的表格項目之
後,同一份登記表篩選出 `group: 'insert'` 的項目(水平線、圖片)。表格項目必須手寫,是因為工具列上的
表格控制項是一個帶尺寸的網格彈出視窗加上一個自訂尺寸對話框,不是單一一個指令,所以它從來沒有加進
另外兩個來源讀取的那份登記表裡。這樣選單總共有十二個項目,順序固定為:Heading 2 到 Heading 6、項目
符號清單、編號清單、引言區塊、程式碼區塊、表格、水平線、插入圖片。

fork 要新增一個項目,該改哪個檔案取決於它屬於哪個來源:多一個標題層級,改 `HEADING_LEVELS`;一個新
的區塊轉換或插入指令,加進 `richTextCommands.ts` 的登記表、標上 `group: 'block'` 或
`group: 'insert'`,就會自動被選單撿起來,`richTextSlashCommands.ts` 完全不用動;像表格項目那樣沒辦法
表達成單一登記表指令的東西,就得直接寫進 `buildSlashItems` 自己的項目清單裡。

**別名。**每個項目至少要有一個別名才搆得到:輸入在 `/` 後面的查詢字串,會以不分大小寫的子字串比對,
同時比對項目翻譯後的 label 與它的別名。標題的別名是就地寫的(`h2`…`h6`、`heading2`…`heading6`);
其他每個從登記表衍生出來的項目,別名都來自 `richTextSlashCommands.ts` 自己的別名表:
...
純 ASCII 狀態。把別名翻譯掉,就會拿掉它存在的唯一理由。
...
Enter 會執行反白的項目;Esc 會關閉選單,而且這完全是 `@tiptap/suggestion` 自己處理掉的,連 extension
自己那個按鍵處理函式都還沒被問到。Tab 是刻意放著不管的——extension 的按鍵處理函式對它回傳
`false`——所以它會照瀏覽器原本的行為往下走,而不是被攔截當成確認選取的手段。如果攔下 Tab,只因為
畫面上剛好有個 `/`,就會把這個欄位從表單原本的 tab 順序裡拉出來。
