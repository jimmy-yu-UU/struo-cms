...
因為 resize node view 自己的容器上限就是欄位本身的文字量測欄寬,圖片永遠不可能被拖曳超過那個
欄寬——顯示出來的寬度(以及放開之後儲存下來的寬度)就是在那裡被限制住的。對一個有固定閱讀欄寬
的欄位來說,這是正常的 WYSIWYG 行為,不是 bug:編輯器在拖曳時顯示的樣子,就是發布之後會渲染出
來的樣子。
