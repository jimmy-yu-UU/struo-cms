...
| `components/` | 直接位於 `components/` 底下的 `ItemForm.vue`(生成出來的項目表單)，加上 `ui/`(供應商 shadcn 原子元件——`button`、`table`、`select`、`dialog`、`sidebar`……——生成輸出；**唯讀**，不得編輯、也不得對它 `:deep()`)、`data/`(`DataTable`、`SortableHeader`、`DataTablePagination`、`FilterBuilder`——`CollectionListView` 賴以建構的 TanStack-table 列表基元)、`fields/`(每個欄位介面各一個編輯器元件，第 5 章，另外還有用於帶 payload 多對多關聯的 `JunctionLinksEditor`，第 7 章)、`common/`(`PageHeader`、`ListToolbar`——都是純 Tailwind/shadcn 元件；`MediaLibraryView` 使用 `ListToolbar` 作為搜尋框與篩選插槽，而 `CollectionListView` 則是建構在 `data/` 的基元之上)、`shell/`(topbar、側邊欄導覽項目、主題切換器、UI 語言切換器、品牌標誌)、`media/`、`revisions/`、`rbac/`。 |
...
寫進 `buildSlashItems` 自己的項目清單裡，不在下面說的那份別名表範圍內——但跟其他每個項目一樣，光靠
它的英文名稱就能搆到，完全不需要別名，只是有兩個條件。第一，它的 `enLabel` 必須來自 `enLabelFor`，
跟表格項目自己那份一樣，而不是用組出這個項目其他欄位的那個翻譯函式解析出來的：`enLabel: t(key)` 就
寫在 `label: t(key)` 旁邊，看起來像是理所當然的填法，但那樣會讓這個值綁定到當下啟用的語言，而不是
綁定到英文。第二，它的 labelKey 必須跟著表格項目的那把一起，加進 `EN_LABELS` 自己組出來的那份清單
裡——`enLabelFor` 只會查在那裡解析過的 key，漏掉這一步的新 key，會在每次選單被叫著要打開時就丟出
例外，而不是像這個檔案裡其他缺漏的 key 那樣在載入模組的當下就丟出來。

**別名。** 輸入在 `/` 後面的查詢字串，會以子字串比對三樣東西：項目翻譯後的 label、這個項目的英文
label，以及它的別名（`filterSlashItems`）。不管後台介面現在是哪種語言，英文 label 這一路比對都成立，
所以一個新加進登記表的指令，完全不用寫別名，光靠它自己的英文名稱就搆得到——例如即使在 zh-TW 介面下、
畫面上顯示的 label 是「編號清單」、本身完全不含 "numbered" 這個字串，`/numbered` 還是能透過英文名稱
找到它，即使 `orderedList` 自己的三個別名（`ol`、`number`、`ordered`）也都沒有一個包含這個查詢字串。
別名存在的目的，是替某些項目再提供一種更快的打法——一個 label 本身沒拼出來的縮寫或同義詞；下面表裡
有好幾個別名本身就已經是自己項目英文 label 的子字串，就算不列在這裡也一樣搆得到。標題的別名是就地寫的（`h2`…
...
純 ASCII 狀態。把別名翻譯掉，就會拿掉它存在的唯一理由。翻譯後的 label 與英文 label 這兩側都不分大小
寫——輸入的查詢字串會先轉成小寫，再拿去跟兩者比對——但別名本身是照原樣比對的，所以上面那張表對 fork
想加的任何別名都是一個真實的限制：別名本身就得已經是小寫，否則查詢字串的大寫版本永遠比對不到它。
...
**捨棄行為。** Esc 跟外部點擊——比如點到另一個欄位——都會讓這次的查詢維持「已捨棄」的狀態：再多打幾個
字也不會重新打開選單，但在別處重新打一個 `/` 仍然照常打開。相對地，失焦——Tab，或是在 Chrome 裡整個
視窗本身失去焦點——一樣會關閉選單，但會釋放查詢而不是捨棄它，所以回到原本的位置、繼續打字，選單會
重新打開；這個區別是靠上游自己的 `shouldResetDismissed` 這個 suggestion 選項做出來的，這裡把它接成
只在由失焦觸發的退出時才生效。
