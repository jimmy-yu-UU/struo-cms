...
**陣列型設定是整個取代內建預設值，而不是併入其中。** 在你自己的設定中設定 `Oidc:Scopes` 或
`Struo:Files:ImageTransform:AllowedFormats`，會完整取代下方列出的內建預設值——而不是加到它上面。
這是本章唯一兩個出貨時就帶有非空內建預設值的陣列型設定;本章其餘的陣列型設定預設值都是空陣列，
不會有這種取代或併入的分別。
