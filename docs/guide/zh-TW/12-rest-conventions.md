...
未攔到的例外會在伺服器端記錄下來，回給呼叫端的一律是固定的遮罩訊息，絕不洩漏例外本身的內容；這件
事，連同認證階段本身失敗的情況，也走同一套遮罩與同一種信封。GraphQL 是唯一的例外：`/graphql` 的回
應是原生的 GraphQL 形狀（`data`／`errors`），完全不套這個信封，除非是像少了 CSRF 標頭這種請求根本
到不了 GraphQL 就先被擋下的情況——那種回應仍然是這裡的信封形狀。

## 狀態碼

大部分讀取與寫入回 `200`，本文帶著結果。`201` 只出現在三個端點：建立項目、建立使用者、上傳檔案，三
者都帶一個相對的 `Location`，指向剛建立那一筆的標準讀取路由。`204` 用在六個地方：項目與檔案的刪
除、檔案的復原、登出、改密碼、撤銷存取權杖，一律沒有 body。

項目從垃圾桶復原是唯一的例外：`POST /api/items/{collection}/{id}/restore` 回 `200`，本文是復原後
...
| `CONFLICT` | 409 | 關聯限制式擋下的刪除，或其他一般 409 |
...
IP 位址的節流，以及改密碼的節流。`SESSION_REVOCATION_FAILED` 代表密碼變更或使用者刪除本身已經成
功，只是後續要撤銷該使用者其他工作階段時失敗，所以有些工作階段可能還活著；它沒有自己的狀態碼，因
此落到 500，但代碼與訊息仍然是專屬的。`SEARCH_UNAVAILABLE` 的訊息固定是「Search is temporarily
unavailable.」，提供者自己回報的原因只會寫進伺服器端的紀錄。

`PAYLOAD_TOO_LARGE` 與 `SEARCH_UNAVAILABLE` 在一般主機上不容易觸發，前者需要真的傳超過上限的位元
組，後者需要一個會主動失敗的搜尋提供者；`INTERNAL_SERVER_ERROR` 則本來就不該刻意示範——會觸發它
的，是一個真正未處理的錯誤。
...
或空物件）就要通過完整驗證；建立沒有這個顧慮，因為沒有既有列可以合併。可翻譯欄位的必填檢查是另一
條路，跟著翻譯內容逐 locale 檢查，不受這條規則影響。

更新時，長度上限與其他欄位層級的檢查會先於這條合併必填檢查跑，所以同一個本文如果同時違反兩者，回
報的是先跑到的那一項，跟建立（先檢查必填）可能不一樣。

一個能指名欄位的錯誤長這樣：`Field 'weight' has an invalid value.`；一個剖析階段就出錯、指不出是
哪個欄位的錯誤，退回泛用的「Request body could not be parsed.」。頂層本文如果根本不是一個 JSON
...
不會退回去試 cookie——呼叫端同時帶了兩種憑證，也只認 bearer 這一種，權杖失效或格式不對，就直接降級
成匿名，而不是改用 cookie 身分。這是刻意的失敗封閉設計。

大部分讀取端點不強制要求任一種憑證，因為讀取只看這個集合的讀取授權；寫入端點則明確認兩種都收，
cookie 或 bearer 皆可。SSO 與密碼規則的完整細節留給認證那一章。
...
算數。這個檢查只在請求真的靠 cookie 認證時才啟動：用 bearer 身分呼叫、或者請求根本沒帶工作階段
...
只有繼承自稽核基底的集合，列上才會有 `version` 這個整數；只實作最基本稽核介面的集合沒有這一欄。有
`version` 的集合，讀取跟列表回應都會帶著它，更新的本文也可以把讀到的值原樣送回；伺服器拿這個值去比
對目前存好的那一列，一致才會真的寫入，不一致就整個拒絕：
...
再試」這條路的依據。本文完全不帶 `version` 也可以，這時就退回原本讀到的值，等於放棄這層保護，但仍然
相容舊的呼叫端。還原一筆快照時，`version` 會先被拿掉才套用，所以還原不會因為快照裡是一個過期的版
本號而白白衝突。`version` 本身也不能透過建立或更新的本文直接指定——它只會被當成並行控制的比對值，
從不被寫進那一欄。

## `POST`／`PUT` 本文能設定什麼

建立與更新只會依照這個集合宣告過的可寫欄位與多對一外鍵來繫結本文，其餘欄位一律被靜靜丟掉，不會回報
錯誤——`id`、`version`、系統管理欄位都是這樣，建立不能自己選 id，更新也不能。唯讀與系統欄位另外會在
繫結後被清成空值，所以本文就算故意用同樣的鍵名夾帶內容，也進不了那些欄位。

`translations` 與多對多關聯的鍵不在這個繫結範圍內，但仍然生效——伺服器會另外從原始本文讀出它們，在
同一筆交易裡套用，細節見多對多的寫入形狀。`Json` 型別的欄位是從本文的原始 JSON 文字直接取值，可以放
進任何 JSON 形狀；沒有翻譯的富文字欄位在驗證之前就先做過消毒，量長度上限與實際存進去的都是消毒後的
HTML。

更新只會覆蓋本文裡真的出現過的鍵，不管是自己的欄位還是多對一外鍵，判斷鍵是否出現時不分大小寫；一個
局部的 `PUT` 因此不會不小心清掉伺服器自己在管、呼叫端根本沒送的欄位。以下幾種常見的本文錯誤都是實
際跑出來的訊息：
...
送 `{"publishedAt":""}` 得到上面這個欄位層級的錯誤；送一個不是物件的頂層本文（例如 `[]`），得
到「Request body must be a JSON object.」；送 `{"name":null}` 到一個必填欄位，得到「Field
'name' is required.」。

並非每個集合都能用這個端點建立。對 `file` 集合送 `POST /api/items/file`，不管本文寫什麼都會被整個
拒絕：
...
檔案要走專屬的上傳端點，細節留給檔案那一章；讀取、更新、刪除 `file` 集合則跟其他集合完全一樣，走這
裡的通用規則。

### many-to-many 的寫入形狀

一個多對多關聯的陣列接受混合清單：每個元素可以是純 id，也可以是一個物件——只在這個關聯的 junction
帶 payload 時才成立，物件是 `{ id, ...payload }`。純 id 只代表成員關係，連結或維持連結，不會動這個
目標既有的 payload 值；物件則是成員關係加上一次合併——它點名的 payload 欄位會被更新，沒點名的維持
原值。
...
同一個 id 在陣列裡出現不只一次時，物件一律贏過純 id，不管誰先出現；兩個物件互相衝突時，後面那個整個
取代前面那個的 payload，而不是合併兩者。當這個關聯宣告了排序欄位，決定順序的是每個 id 第一次出現的
位置，跟最後誰贏得 payload 無關。物件裡沒點名的 payload 鍵，還有 junction 自己的結構欄位，一律被忽
略；重複的 id 也只是被去重，不會報錯——junction 本來就是一個集合。

物件元素多一項門檻：呼叫端必須擁有 junction collection 本身的寫入授權，junction 是 `AdminOnly` 時
還要是超級管理員，這一關會在任何 payload 值被解析之前就先擋下。純 id 的元素不受這個限制，只需要這
個關聯所屬集合原本的寫入授權。還原一筆帶 junction payload 的快照時，套用邏輯跟直接寫入完全一樣——
細節見[第 9 章：版本紀錄與軟刪除](09-revisions-and-trash.md)——因此也需要同一個 junction 授權。
