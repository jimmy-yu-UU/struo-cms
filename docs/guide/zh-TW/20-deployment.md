...
`MigrationRunner.ApplyAsync` 把設定值原封不動丟給 `Directory.Exists`，不像
`Struo:Files:ImageTransform:CachePath` 那樣先解析成 `ContentRootPath` 底下的路徑。相對路徑
因此解析到的是「行程啟動當下的工作目錄」，不是應用程式自己的資料夾——systemd 的
`WorkingDirectory`、容器的 `WORKDIR`，或任何先切換目錄才啟動的啟動器，都可能跟它不一樣，指到
的目錄不存在就會讓啟動直接失敗（見下方〈啟動時會快速失敗的事〉）。

### 第一次啟動前就換掉 bootstrap 密碼

`Auth:BootstrapAdmin:Password` 只在 `users` 資料表第一次建立時被讀一次，之後重開行程再怎麼改
這個值，都不會影響已經存在的帳號。用預設值在 Production 啟動只會記一筆點名這個設定鍵的
`WARNING`，不會擋下啟動；這道警告比對的是目前設定裡的值，不是資料庫裡實際存的雜湊，所以先讓
它用預設值種子、之後才改設定，警告會消失，但帳號密碼仍然是預設值那組雜湊——第一次啟動前就換
掉它，才是真的換了密碼。

### 登入速率限制：帳號層開、IP 層看拓樸

帳號層的節流（`RateLimiting:LoginAccount`）預設開著，能單獨擋下針對單一帳號的暴力嘗試；IP 層
的節流（`RateLimiting:Login`）預設關閉，在反向代理後面打開它，所有使用者會因為
`Connection.RemoteIpAddress` 變成代理自己的位址而收斂成同一個桶（原因見下一項）。兩層各自的行
為見[第 16 章：認證與 SSO](16-authentication.md)〈登入速率限制〉一節。
...
完全不存在；相鄰的 GraphQL schema 揭露路由另外由 `GraphQl:ExposeSchema` 控制，預設值 `null`
同樣只在 Development 開放，機制見[第 14 章：GraphQL API](14-graphql.md)。

### OIDC 三個檢查要釘死

`Oidc:AllowedTenantId` 的預設值是一個打不中任何 tenant 的佔位字串，換成真正的 tenant 之前，
外部登入對任何人都會失敗；換上真正的 tenant 之後，`Oidc:RequireEmailVerified`（預設不要求）
...
另一件跟設定無關、但同樣在啟動最上層就決定好的事：程序一開始就把目前執行緒的文化特性釘死成
...
後台 SPA；沒有正式環境用的 `docker compose` 檔。本機建置跟 CI 的 `docker` job 是同兩道指
令，只是本機習慣打 `:local` 標籤，CI 打的是 `:ci`：
...
`ASPNETCORE_ENVIRONMENT` 這個映像沒有另外設定，不設就是 ASP.NET Core 自己的框架預設值
...
享的金鑰存放區（共享檔案系統、Redis，或雲端供應商的金鑰圈服務）——這個專案本身沒有配置任何
一種，這是一個誠實的限制。

### 後台 SPA 映像

監聽 `80`，用 nginx 提供 Vite 的 production build。跟 API 映像不同，這裡沒有
`HEALTHCHECK`——沒有下游依賴可以探測，Dockerfile 自己的註解說明了原因：要探測就直接打 `/`。

建置階段會吃進一份已提交的 `frontend/.env.production`（`frontend/.env.example` 示範了
`VITE_API_BASE_URL`）；這是 Vite 自己 `vite build` 的行為（production 模式預設讀取
`.env.production`），不是 Dockerfile 特地實作的——`frontend/.dockerignore` 的註解也確認了它
故意不排除已提交的 `.env.production`，只排除開發者自己的 `.env`／`.env.*.local`。
...
兩個映像都不終結 TLS、都不設定 `UseForwardedHeaders`；重啟策略、水平擴展、密鑰注入、健康檢
查怎麼接進編排系統，都不是任何一個 Dockerfile 的責任——沒有正式環境用的 `docker compose` 檔
可以照抄。
...
探針該在這裡沒回應時重啟容器或 pod。`/health/ready` 執行兩項檢查，兩個都標了 `ready`：
`DbReadinessCheck` 對資料庫做一次真正的連線往返，`CacheReadinessCheck` 對設定好的
`IDistributedCache`（Redis 或行程內快取）做一次寫入再讀回；readiness 探針該等兩項都能連
上，才把流量導進來。兩個路由怎麼打，[第 3 章：快速開始](03-getting-started.md)已經介紹過；
下面是對一個匿名請求的完整回應：
...
```
$ curl -i http://localhost:5221/health/ready
HTTP/1.1 200 OK
Content-Type: text/plain
Date: Thu, 17 Sep 2026 03:24:32 GMT
Server: Kestrel
Cache-Control: no-store, no-cache
Expires: Thu, 01 Jan 1970 00:00:00 GMT
Pragma: no-cache
...
兩個回應的 body 都只是單字 `Healthy`，都帶著 `X-Content-Type-Options: nosniff`——這是這個應
用程式唯一自己送的安全標頭，正式環境檢查清單裡已經提過。
...
- **資料庫**——PostgreSQL 是除了上傳檔案之外一切的系統記錄，用一般的 PostgreSQL 工具備份
  （`pg_dump`、`pg_basebackup`，或托管服務自己的快照／PITR），StruoCMS 沒有取代這套慣例的
  東西。
...
`docker compose --profile s3 up -d`。`createbuckets` 是一次性的：啟動時建好 bucket
`struo-media` 就結束，結束代碼 `0`，`docker compose ps -a` 才看得到它（顯示
`Exited (0)`），`docker compose ps` 預設會把它濾掉。連接埠由 `STRUO_MINIO_PORT`（預設
9000，API）與 `STRUO_MINIO_CONSOLE_PORT`（預設 9001，主控台）兩個環境變數決定，改了
`STRUO_MINIO_PORT` 記得同步改 `appsettings.Development.json` 裡的 `Struo:Files:S3:Endpoint`。

## 接下來

部署談到這裡；資料庫的結構怎麼分層管理、跨 fork 怎麼升級核心，見
