...
桶，可翻譯 leaf 的附屬資料表查找也不會再顯示一個已被移入回收桶目標的翻譯內容。多對一外鍵形態
(`categoryId`) 是唯一的例外:它是對根集合自己的欄位做 facet，不是走關聯，所以一個已被移入回收桶
目標的 id 仍然會出現在裡面。這個過濾動作是在數值上限 (下方「排序與數值上限」一節) 已經把 id 桶
截頂**之後**才執行的，所以如果被保留下來的前幾名 id 之中，有一個或多個所指的目標後來被移入了
回收桶，回應中的桶數就可能少於這個上限。
