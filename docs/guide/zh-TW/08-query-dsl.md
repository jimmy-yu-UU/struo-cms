...
`deleted=with` 請求的 `tags` facet，仍然會排除一個已被丟進回收桶的標籤，即使文章資料列本身包含了
已回收的項目。
...
一起被剪掉。`search` 永遠不會被剪；**彙總永遠不會被剪**——它一律針對請求完整、未剪枝的
...
`category.name` 涵蓋了這個 host 上的每一個分類。彙總則完全無視剪枝：`publishedAt` 上的
...
再加上葉值查找）；彙總每 10 個請求的 op/欄位 slot 多加 1 條陳述式，以固定常數
...
fixture 分類裡兩篇已發布的文章上的 `count`/`max`（第三篇還是草稿，所以它的 `publishedAt` 是
`null`，不計入）：
