...
`[CmsRelation]` 沒有建構子，所有設定都用具名參數寫。前四個決定怎麼顯示、怎麼挑目標：
...
`Cascade` 連帶清掉的還有那些列各自的 junction、翻譯與版本紀錄，用一個 visited 集合防止循環——循環參
照不會讓它無限遞迴。參照它的列讀取時繞過軟刪除篩選，已經在垃圾桶裡的參照者一樣會被清掉，不會被跳過。
