...
符合 `^[1-9][0-9]{0,4}$`(頭尾都有錨點)時才算數。這接受的是一到五位、沒有前導零的 ASCII 數字——
...
裡，理由跟 `rel` 需要獨立處理常式一樣:允許清單只能表達「這個名稱被允許」，沒辦法表達「只在這個
標籤上被允許」。
...
這個收窄規則是集中在一個地方執行的政策選擇，不是不變量:`GanssHtmlSanitizer.cs` 自己的註解完整
說明了長寬比背後的理由，fork 若想接受百分比寬度，或連 `height` 一起儲存，應該直接在那裡修改——
允許清單本身沒辦法表達「限定標籤」或「限定形狀」這類規則，所以那個檔案是唯一能改的地方。
