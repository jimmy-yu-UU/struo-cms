...
| `RateLimiting:Password:Enabled` | 只有在多 pod 部署中、逐使用者的全域上限不重要時，才設為 `false` | 改密碼限流器依**已驗證呼叫端的使用者 id** 分區，不是依 `Connection.RemoteIpAddress`，所以它**不會**有上面那一列描述的、反向代理把所有請求收斂成單一共用桶的失效模式。不過它仍然有登入限流器的另一種失效模式:它的計數器是位於處理程序內、逐 pod 的記憶體(`Program.cs`、`AddPolicy("password", …)`)，所以在 N 個 replica 背後，同一個使用者的請求可能落在不同的 pod 上，讓有效的逐使用者上限變成大約設定值的 N 倍，而不是一個真正的全域上限。 |
