...
through an external identity provider. Which of cookie or bearer a caller should pick, and when
the CSRF header is required, is covered in
[Chapter 12: REST API Conventions](12-rest-conventions.md); this chapter covers the mechanics
behind each credential — how it's issued, where it lives, when it expires, and how it's revoked.
What a caller can do once authenticated is covered in
...
so the deliberately short default password in the settings file still works for that first login.
The transcripts in this chapter log in with that same default password; a production environment
should change it immediately after the first login.
...
producing a self-contained PHC-encoded string. The salt and the parameters are both embedded in
the hash itself; the `user` collection carries no separate salt column.
...
account exists, or the account has no local password at all (the OIDC section below covers this
kind of account), the server still runs one full Argon2id verification against a fake hash, so
failure takes the same amount of time either way — the caller can't infer from response speed
whether that email exists.
...
`CONFLICT` `Email already in use.`. The password's length limits and their configuration keys are
in the `Auth` section of [Chapter 4: Configuration Reference](04-configuration.md).
...
verifies the current password, a super-admin's change on someone else's behalf never does. A
wrong current password returns `400` `INVALID_CURRENT_PASSWORD`; a non-admin trying to change
someone else's password returns `403` `Admin role required.`; a new password that violates the
policy returns `400` with a field-level message. An account that only ever logged in through OIDC
and never set a local password is stopped before the current-password check even runs, when it's
the owner calling: `400` `NO_LOCAL_PASSWORD`. A super-admin can still set a local password on such
an account. Across the whole identity surface, this is the only endpoint that lets a
non-super-admin write at all.

This endpoint carries its own rate limit, partitioned by the calling caller's own user id — not
the target user's, and not the caller's IP: a super-admin resetting several users' passwords in a
row spends its own quota, and never locks out the accounts it's changing on their behalf. The
...
fixed to `Always`; every other environment follows the request itself. As soon as any CORS origin
is configured at all, both the `Secure` policy and `SameSite` switch to `Always`/`None`, because a
browser only accepts `SameSite=None` under `Secure` — meaning a cross-origin deployment has to run
HTTPS on both sides.

The cookie itself is only an opaque key; the actual session state behind it lives in a
distributed cache: setting `Redis:ConnectionString` uses Redis, and leaving it unset falls back to
an in-process cache — the practical difference is that restarting the process loses every session,
and replicas don't share state with each other; Redis holds up under both conditions. Expiry is
the same 8-hour sliding window everywhere: the cookie itself, the cache entry, and the index row
...
session behind that key is deleted from the cache too, not just this one request blocked. This
check, along with the ordinary `401` `Authentication required.`/`403` `Forbidden.` denial when
sign-in is required but no credential is present, is written out by the cookie handler before the
MVC action itself ever runs; the action needs no handling of its own.

The default authentication scheme isn't actually the cookie itself, but a forwarding scheme: a
request carrying `Authorization: Bearer …` forwards to bearer authentication, and only otherwise
forwards to the cookie — which is why even reads, `/graphql`, and file reads, none of which
declare any scheme of their own, still recognize a bearer caller. On these scheme-less endpoints,
a broken or revoked token always degrades to anonymous rather than falling back to try the cookie,
even when the request carries both at once; a write endpoint that requires sign-in validates and
merges both schemes instead — see [Chapter 12: REST API Conventions](12-rest-conventions.md).
...
recorded in the index row doesn't change along with it. The next section covers how user-level
revocation (a password change, an account deletion) relies on that same index, so a session
renewed in place is only revoked by the previous user's password change or deletion, never by the
new user's. Log out before switching accounts, rather than letting the same cookie jar answer for
...
cache, and clearing the browser-side cookie. It revokes only the cookie side: a bearer token the
same account also holds is completely unaffected — a request authenticated only by bearer still
gets `204` from this same endpoint, and its token stays alive. A token can only be revoked through
the next section's revocation endpoint, or by issuing a new one that overwrites the old hash.

`user_sessions` is a key-to-user index table, and the reason it exists is direct: the distributed
cache itself has no scan or enumerate operation, so without this index there's no way to answer
"which sessions does this user currently have alive." It isn't a collection browsable or editable
...
revocation, and it's done at the service layer, so GraphQL's delete mutation is covered by the
same path.
...
only once, at the moment it's issued, and only its hash is kept afterward. The raw value is 256
bits of random data, base64url-encoded with padding stripped; what's stored is its uppercase
...
Whether the account doesn't exist, the password is wrong, or the account is deactivated, each
counts as one failure, and a blocked request always returns the same `429`, so the throttle itself
can't be turned into a tool for guessing whether an account exists; a successful login clears that
account's count.
...
Either layer being tripped returns `429`, with the envelope code always `TOO_MANY_REQUESTS` and
...
throttle; what's shown is the response to one more request sent after the throttle has already
engaged:
...
It's the authorization code flow with PKCE; the provider's own token is never stored, claim names
use the short form, and a separate call to the userinfo endpoint fills in the rest. The callback
path is `Oidc:CallbackPath`, defaulting to `/signin-oidc`, and that's the redirect URL to register
with the provider. The requested scopes are `Oidc:Scopes`, defaulting to `openid`, `email`,
`profile` — leaving it blank or giving an empty array both fall back to that same default, never
to no scope at all. The claims it reads:

- `email`, falling back to the longer email claim type, then to `preferred_username`, when missing;
...
account beforehand. A JIT-provisioned account is active, role-less, and its password field is
empty; having no role means it has nothing beyond the `public` floor — external login resolves
identity, not authorization; roles and that floor are covered in
