...
| `Oidc:RequireEmailVerified` / `AllowedTenantId` / `AllowedEmailDomains` | Pinned explicitly whenever `Oidc:Enabled=true` | All three default permissive (`false`, empty/placeholder, `[]`). `ExternalLoginService.ResolveOrProvisionAsync` (`src/Struo.Application/Security/ExternalLoginService.cs`) links or logs in by **email equality alone** once those checks pass — with the defaults, that is the *only* real check. Any external identity provider account whose claimed email happens to match an existing local user's email is treated as that user, tenant and verification status notwithstanding. |
...
- **`backend`** — `dotnet restore`, `dotnet build --configuration Release`, then
  `dotnet test --configuration Release` — the full backend unit/integration suite. No
