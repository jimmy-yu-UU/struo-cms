...
| `RateLimiting:LoginAccount:Enabled` | `true` (the shipped default) for essentially every deployment | `AuthController.Login` checks `ILoginAttemptThrottle` before spending any Argon2id CPU (`src/Struo.Infrastructure/Identity/DistributedCacheLoginAttemptThrottle.cs`), throttling by the ACCOUNT named in the request body rather than by client IP. Turning it off leaves a distributed, slow password-spray against one account (many source IPs, one target account) completely unthrottled, since `RateLimiting:Login` below cannot see the request body and has no way to key on the account being attempted. Unlike the other two rate limiters in this table, whether this one is per-pod or global depends on `Redis:ConnectionString`: configured, it shares the same `IDistributedCache` as the session-ticket store and enforces a true global per-account limit across every replica; left empty in a multi-replica deployment, it silently degrades to the same in-process, per-pod cache the ticket store falls back to, and the effective limit becomes ≈N× the configured value like the other two. |
