...
The admin SPA binds the effective `MaxLength` directly to the native input's `maxlength` attribute,
so
...
and `Uuid` send `null` when left blank (`number`, `slider`, `rating`, `boolean`, `checkbox`, `uuid`
); text interfaces deliberately send an empty string, so clearing a text field actually clears it.
