...
silently rewrite the snapshot instead of checking it. Non-string fields such as numbers, booleans,
and `Uuid` send `null` when left blank (`number`, `slider`, `rating`, `boolean`, `checkbox`,
`uuid`, among others); text interfaces deliberately send an empty string, so clearing a text
