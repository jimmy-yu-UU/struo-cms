...
| Batch 5 | LOW 批次（後端/資料/前端/測試） | ✅ 完成（branch `audit-2026-07-21-batch5`；commits `f3c69fc` 後端/db + `390d0d6` 前端 + doc）。**(1) migration 核心重整**：13 支範例糾纏 migration → 單一 `001-core-baseline.sql`；BL-5/SEC-11/DB-20 決策定案；live PG gate PASS。**(2) LOW 尾批**：BL-2/BL-4/SEC-9/AUTH-2、DB-16(part1 冗余btree)/DB-17/DB-18/DB-19、SEC-12/FE-20~30、TEST-7/8/9 全數修復；**DB-21 skipped**（預讀=partial-PUT 合併基底，非免費效能）；**DB-16 part2（lower(email)/contenttype 效能索引）依使用者決策延後**（避免 prod-only 漂移）；ARC-8 延後。Sonnet ×4 實作 + Fable ×3 對抗複核（全數 no-FAIL，nits 已修）。後端 `dotnet test` **819 綠**、前端 vitest **539 綠**、`pnpm build` 通過；**live PG+MinIO gate PASS**（DB-19 restore 無 42804/冪等、DB-17/18、SEC-9 byte 吻合；DB-16 parity zero-diff）。新增追蹤 SEC-15/DB-22（見執行紀錄）。 |
...
- [x] **AUTH-1 LOW → 已修（2026-07-22）— attribute 層 401/403（`[Authorize]` challenge）回空 body、無錯誤 envelope** ✅ 已修（`AuthWiring.cs` `OnRedirectToLogin`/`OnRedirectToAccessDenied` 於設 status 後以 `WriteAsJsonAsync(Envelope.Error(...), EnvelopeJsonOptions)` 寫入 envelope，code=UNAUTHORIZED/FORBIDDEN、訊息比照 `EnvelopeResultFilter.DefaultMessage`；`EnvelopeJsonOptions=new(JsonSerializerDefaults.Web)` 因 `WriteAsJsonAsync` 解析的是 `Http.Json.JsonOptions` 而非 MVC 的）。**衍生修正**：`Cookies,Bearer` 多 scheme 下 `AuthorizationMiddlewareResultHandler` 依序 challenge 兩者，Cookie 寫 body 後 Bearer 預設 handler 撞 "response already started"（9 測試紅）→ `BearerTokenAuthenticationHandler` override `HandleChallengeAsync`/`HandleForbiddenAsync` 加 `if (!Context.Response.HasStarted)` guard。TEST-3 翻轉為斷言 `error.code=="UNAUTHORIZED"`。`dotnet test` 818 綠。Fable **PASS-WITH-NITS**（scheme-order 分析確認 Cookie 恆先、envelope 必達；bearer-only 端點目前不存在故無回歸）。殘留 LOW nits（未修，latent 無觸發）：EnvelopeJsonOptions 無 enum converter（今無 enum 欄位）、cookie 事件未加 HasStarted guard（防禦性）、未來若加 bearer-only `[Authorize]` 其 401 將無 envelope。
- [ ] **AUTH-2 LOW（新，複核 AUTH-1 時揭露）— CSRF 403 回非 envelope 形狀 body（`{error:{message}}`，缺 `success:false` 與 `error.code`）**
  `src/Struo.Api/Auth/CsrfProtectionMiddleware.cs:28-30` 自訂 body。AUTH-1 修完後，這是唯一仍非 envelope 形狀的 auth-adjacent 錯誤路徑；前端若依 `error.code` 分支會拿到 undefined。**修法（Batch 5）**：比照 AUTH-1 用 `Envelope.Error(ErrorCodes.Forbidden, ...)` 寫入。
- [ ] **FE-30 LOW（新，複核 media e2e 時揭露）— MediaDetailDialog 於 `load()` in-flight 期間 Title/Alt 輸入未 disable → 快打字落入 `translations['']` 遭清空**
  `MediaDetailDialog.vue:192,196` 無 `:disabled="loading"`；dialog `visible` 早於 `load()` 完成（`activeLocale` 初始 `''`）。e2e 已用 `waitForResponse` 規避（非弱化），但元件 UX race 仍在。**修法（Batch 5）**：load 期間 disable/skeleton 欄位。
- [x] **E2E-1 → 已修（2026-07-22）— 預設 UI locale=zh-TW + Playwright 空 localStorage → i18n 字串 selector 全失敗** ✅ 已修（新增共用 `frontend/e2e/fixtures.ts`：`base.extend` 覆寫 `page` fixture，於 `use(page)` 前 `addInitScript` seed `localStorage['struo.uiLocale']='en'`；全 11 支 spec 改由 `./fixtures` 匯入 `test`/`expect`，`type Page` 仍自 `@playwright/test`；移除 3 支新 spec 的 per-login 暫解）。Fable **PASS**。
- [x] **E2E-2 LOW → 已修（2026-07-22）— 既有 gold specs 自 FE-R0/R1/R3 改版後選擇器漂移** ✅ 已修（4 支：`auth` logout 移入 UserMenu popover→Account 鈕+Log out menuitem；`collections` sidebar 非 PanelMenu 且群組預設展開→單擊 `aside.sidebar` 的 Article 鈕、Status 欄仍成立；`not-found` i18n `'Item not found'` 去句點；`unsaved-guard` `navSidebar` `locator('nav')`（實為 breadcrumb）→ `aside.sidebar`+button role，並移除不健全的展開 fallback）。Fable **PASS/PASS-WITH-NITS**（無弱化斷言，選擇器逐一對照現行元件/i18n）。純測試選擇器修正，無 production 改動。

### Batch 4 live e2e gate — ✅ 全數 PASS（2026-07-22 實跑，backend :5080 limiter-off + vite :5173 + 本機 dev PG/MinIO/Redis，dev super-admin 登入）
- **`pnpm exec playwright test --workers=1` → 17/17 全綠**（auth/collections/conflict×2/items/media/not-found/relations×2/revisions/settings×2/trash/unsaved-guard×4）。單 worker 為必需（單一共用 dev server + 共用登入；平行會 goto 逾時且撞 SEC-7 login limiter）。
- **關鍵環境要點（下次實跑必看）**：(1) e2e 預設帳密 `admin@struo.local/change-me-please` 非本機 dev seed；本機用 `E2E_EMAIL=admin@admin.com E2E_PASSWORD=admin#90196080`（dev `BootstrapAdmin`）。(2) SEC-7 login limiter dev 預設 `Enabled=true PermitLimit=5/60s`→17 次登入必觸 429；實跑須 `RateLimiting__Login__Enabled=false`（比照整合測試 ApiFactory 設高限額）。(3) vite v8 綁定問題→用 `pnpm dev --host` 綁全介面使 localhost/127.0.0.1 皆可，且先暖機。(4) 後端 `--no-launch-profile` 避免 launchSettings 覆寫成 :5221。
- media spec 實跑揭 2 個真 bug（已修，非弱化）：tile 內 `.file-chip__name`+`.media-tile__name` 同名雙節點 strict violation→ scope 到 `.media-tile`；dialog `visible` 早於 `load()`→ 加 `waitForResponse(GET /items/file/)` 再互動。
...
- **殘留 Batch 5 LOW（未做，下一份計畫）**：BL-2/BL-4/SEC-9/SEC-12/AUTH-2、DB-16(核心索引+冗余btree)/DB-17/DB-18/DB-19/DB-21、FE-20~30、TEST-7~9；ARC-8 明示延後。 → **已於下方 LOW 尾批完成（2026-07-23）**。

### Batch 5 執行紀錄 — LOW 尾批（2026-07-23，branch `audit-2026-07-21-batch5`）
- **流程**：4 個 Sonnet 5 子代理平行/分波實作（叢集 A 後端核心 BL-2/BL-4/SEC-9/AUTH-2+TEST-7/8/9、叢集 B 資料庫 DB-17/18/19、叢集 C 前端 SEC-12/FE-20~30；三者檔案不相交，A 與 C 平行、B 因與 A 共用 .NET build 序後跑）+ DB-16 由 orchestrator 親自處理（觸及剛驗證的基線）；3 個 Fable 5 子代理對抗複核（依叢集分組，**全數 no-FAIL**，nits 已全修）。
- **完成項**：
  - **BL-2** ✅ `FileAccessPolicy` adopt bearer principal 後 `GetCurrentUserId() is null → 拒絕`（public floor，不落穿）；補單元測試以 fake `IAuthenticationService` 直驅該 branch（今日 production 不可觸發，純防禦）。
  - **BL-4** ✅ 三個 DI 擴充方法移除未用的 `IConfiguration` 參數；5 處 call site（Program.cs ×3 + 2 測試檔）更新；`.BindConfiguration` 仍由 DI 解析,行為不變。
  - **SEC-9** ✅ 上傳改 `FileBufferingReadStream`（memoryThreshold 64KB，逾量落 temp）取代無條件 `MemoryStream`；**額外補正**：`FileBufferingReadStream.Length` 未汲乾前不反映總長 → 存檔前顯式 `CopyToAsync(Stream.Null)` drain 使 S3 Content-Length 正確；magic-bytes 讀取後 rewind、全路徑 dispose 皆確認。套件 `Microsoft.AspNetCore.WebUtilities 10.0.10` 經 `dotnet add package` 進 CPM。
  - **AUTH-2** ✅ `CsrfProtectionMiddleware` 403 改回標準 `Envelope.Error(ErrorCodes.Forbidden,…)`+`WriteAsJsonAsync`（比照 AuthWiring）；alias `using ErrorCodes = Struo.Api.Http.ErrorCodes` 避與 HotChocolate 撞名；TDD 先寫紅（測試須剝掉 harness 預帶的 CSRF header 才打得到拒絕路徑）。
  - **DB-16（part 1）** ✅ 移除 `FileTranslation.cs` 冗余 `[SugarIndex] ix_file_translations_fk_locale`（唯一索引已覆蓋同 (fileid,locale)）；`001-core-baseline.sql` 外科式刪該 btree（git diff 僅 2 行）；`IndexParityTests` 重定向（移除該 entity/索引名）。**parity 以 gold-standard 證明**：對乾淨 PG 跑 `InitTables(FrameworkEntityTypes.All)` 產生的核心 schema 與手改基線索引集 **byte-identical（各 19 索引、zero diff）**；基線套空 DB 冪等、9 表、file_translations 僅存 pkey+unique。
  - **DB-16（part 2，lower(email)/files.contenttype 效能索引）** — **依使用者決策延後**（2026-07-23）：functional/pattern 索引無法由 InitTables 產生 → 只能進 prod-only `002-…`，會重開剛消除的 dev/prod 漂移；且模板尚無 prod 資料（純理論效能，YAGNI §0）。留待未來有部署與查詢熱點時的專門效能 pass。
  - **DB-17** ✅ `QueryTranslationParentIdsGenericAsync` 改 SQL 層 `.Select` 只投影 FK（新 `QueryTranslationFkSelectGenericAsync<T,TFk>` 依 runtime FK CLR 型別二次泛型分派，快取鍵 `(EntityType,FkType)`；避開 `Expression.Convert(→object)` 被 SqlSugar 靜默丟棄的坑，既有測試釘住）；`RelationFilterResolver` 僅委派故不需改。
  - **DB-18** ✅ `SqlSugarRevisionStore.ListAsync` 匿名投影只取 number/operation/createdat/createdby；`RevisionInfo` 本就不帶 Snapshot → list JSON 不變；detail 路徑仍回完整快照。
  - **DB-19** ✅ soft-delete restrict 檢查移進 txn（比照 purge 邊界）；restore 改原子 `WHERE deletedat IS NOT NULL` + 僅 `affected>0` 記 revision（消除 restore 側 TOCTOU 重複記錄）；typed-NULL entity-typed `SetColumns` 逐字保留（42804-safe）。
  - **DB-21** — **skipped（不安全）**：`UpdateCoreAsync` 的存在性預讀其實是 partial-PUT 的合併基底（保住 body 未帶的伺服器管理欄位），移除是行為變更非免費效能；且 404/409 分流由 repo 自身檢查+CAS 支撐,不依賴此預讀。純效率 LOW 不值得改語意。
  - **SEC-12/FE-20~30** ✅ 見前端 commit `390d0d6`（FE-20 token 修正含複核追加的 `FilesField.vue` 漏網 `--text`；FE-22 `lib/toFileRow.ts`；FE-24 loadPromise 失敗清除；SEC-12/FE-30/FE-26 MediaDetailDialog toast+load 期間 disable+測試；FE-25 鍵盤 a11y；FE-27 reload()；FE-28/29 移除 prop+dots aria-label）。
