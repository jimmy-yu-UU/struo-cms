...
public sealed class ItemDeserializer(IEntityRegistry registry, IM2MDescriptorSource m2mSource, RichTextCleaner richText)
{
        ...
        string collection, JsonElement body, CollectionMetadata meta, bool enforceRequired, IReadOnlySet<string>? onlyFields)
    {
        ...
        var stripNames = ComputeStripNames(collection, meta, body, onlyFields);
        var entity = DeserializeEntity(d, body, stripNames);

        SetJsonFieldRawText(meta, body, d, entity);
        ClearSystemReadOnlyFields(meta, d, entity);
        SanitizeRichTextFields(meta, d, entity);

        // Skipped entirely for a partial bind (enforceRequired: false) — a junction payload object
        // only sets the fields it names, so a required payload field absent from this particular
        // element is not an error; absent payload fields keep their stored value.
    ...
    // fail trying to convert a long into the target entity type. Also strips the translation
    // sidecar payload, Json-interface fields (bound from raw text below instead), and — via the
    // allowlist — any key metadata never declared as writable.
    private HashSet<string> ComputeStripNames(
        string collection, CollectionMetadata meta, JsonElement body, IReadOnlySet<string>? onlyFields)
    {
        ...
        var allowedKeys = ComputeAllowedKeys(meta, onlyFields);
    ...
    private static HashSet<string> ComputeAllowedKeys(CollectionMetadata meta, IReadOnlySet<string>? onlyFields)
    {
    ...
    private static object DeserializeEntity(EntityDescriptor d, JsonElement body, HashSet<string> stripNames)
    {
        if (stripNames.Count == 0) return DeserializeElement(body, d.EntityType);
        ...
        return DeserializeElement(stripped.RootElement, d.EntityType);
    }

    private static object DeserializeElement(JsonElement element, Type entityType)
    {
        try
        {
    ...
    private static void SetJsonFieldRawText(CollectionMetadata meta, JsonElement body, EntityDescriptor d, object entity)
    {
    ...
    private static void ClearSystemReadOnlyFields(CollectionMetadata meta, EntityDescriptor d, object entity)
    {
    ...
    private void SanitizeRichTextFields(CollectionMetadata meta, EntityDescriptor d, object entity)
    {
    ...
    private static void EnforceMaxLength(CollectionMetadata meta, EntityDescriptor d, object entity)
    {
